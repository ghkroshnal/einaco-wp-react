<?php namespace hide_site_title_BillCatchErrors;
// created 06/23/23
...
function hide_site_title_bill_hooking_catch_errors()
{
        if (function_exists('is_admin') && function_exists('current_user_can')) {
            if(is_admin() and current_user_can("manage_options")){
                ...
                $hide_site_title_plugin_slug = 'restore-classic-widgets';
...
add_action("wp_ajax_bill_minozzi_js_error_catched", "hide_site_title_BillCatchErrors\\bill_minozzi_js_error_catched");
add_action("wp_ajax_nopriv_bill_minozzi_js_error_catched", "hide_site_title_BillCatchErrors\\bill_minozzi_js_error_catched");
    ...
    if (isset($_REQUEST)) {
        ...
        foreach ($errors as $error) {
            ...
            if($log_error) {
                ...
                if($hide_site_title_plugin_slug  == 'wptools')
...
class hide_site_title_bill_catch_errors
{
...
new hide_site_title_bill_catch_errors();
//
